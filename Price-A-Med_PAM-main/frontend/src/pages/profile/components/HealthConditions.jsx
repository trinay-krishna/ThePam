import HealthConditionsForm from "./HealthConditionsForm.jsx";
                ...
                <div className="flex gap-1 flex-col">
                    {item.conditions.length === 0 ? (
                        <div className="flex justify-center h-[60px]">
                            <img src="https://res.cloudinary.com/dkezdazmt/image/upload/v1737902486/Evernorth/history.svg" alt="No conditions" />
                        </div>
                    ) : (
                        item.conditions.map((conditionObj, index) => (
                            <div key={index} className="flex justify-between items-center hover:bg-gray-100">
                                <span>{conditionObj.name}</span>
                                {/*<img*/}
            ...
            {form && (
                <div className="fixed top-0 left-0 right-0 bottom-0 z-50 h-screen w-full flex items-center justify-center">
                    <div className="w-[35%]  bg-[#035c67aa]  p-4 rounded-lg border-2 border-[#035c67] shadow">
                        ...
                        <HealthConditionsForm initialSelectedConditions={data.conditions} />
                    </div>
                </div>
            )}
