...
const UpdateDependentForm = ({initialDataSection}) => {
  ...
  useEffect(() => {
    if (initialDataSection.allergies && initialDataSection.allergies.length > 0) {
      setSelectedAllergies(initialDataSection.allergies);
    }
  }, [initialDataSection.allergies]);
  ...
  useEffect(() => {
    if (initialDataSection.medications && initialDataSection.medications.length > 0) {
      setSelectedMedications(initialDataSection.medications);
    }
  }, [initialDataSection.medications]);
  ...
  useEffect(() => {
    if (initialDataSection.healthConditions && initialDataSection.healthConditions.length > 0) {
      setSelectedConditions(initialDataSection.healthConditions);
    }
  }, [initialDataSection.healthConditions]);
  ...
  const updateDependent = async () => {
    const payload = {
      dependentID: initialDataSection.dependentID,
    ...
    try {
      const response = await axios.post(
          `${import.meta.env.VITE_BACKEND_URL}/auth/user/mps/updateDependent`, // Change to your endpoint
          payload, {
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-center w-full">
          <label
              ...
              className="p-1 flex flex-col items-center justify-center w-[60%] h-20 border-2 border-black border-dashed rounded-lg cursor-pointer bg-[#3EFFC0] hover:bg-[#3EFFC0CC]" //dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600"
          >
            {imagePreview != null && (
        ...
        <div className="px-12">
          ...
          <input
              ...
              // className="text-sm mx-12 w-[80%] p-1 rounded px-2 border-2 border-[#035c67] bg-gray-100 text-[#035c67]"
...
export default UpdateDependentForm;

UpdateDependentForm.propTypes = {
  initialDataSection: PropTypes.shape({
    allergies: PropTypes.arrayOf(
        PropTypes.shape({
          allergyID: PropTypes.number.isRequired,
          name: PropTypes.string.isRequired
        })
    ),
    healthConditions: PropTypes.arrayOf(
        PropTypes.shape({
