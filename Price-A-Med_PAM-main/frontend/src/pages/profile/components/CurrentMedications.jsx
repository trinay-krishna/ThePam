...
import CurrentMedicationsForm from "./CurrentMedicationsForm.jsx";

const CurrentMedications = ({data}) => {
                ...
                <div className="flex gap-1 flex-col">
                    {item.medications.length === 0 ? (
                        <div className="flex justify-center h-[60px]">
                            <img src="https://res.cloudinary.com/dkezdazmt/image/upload/v1737902486/Evernorth/history.svg" alt="No medications" />
                        </div>
                    ) : (
                        item.medications.map((medicationObj, index) => ( // Access the medication object
                            <div key={index} className="flex justify-between items-center hover:bg-gray-100">
                                <span>{medicationObj.name}</span> {/* Display medication name */}
                                {/*<img*/}
                                {/*    className="w-4 mr-1 hover:scale-105 "*/}
            ...
            {form && (
                <div className="fixed top-0 left-0 right-0 bottom-0 z-50 h-screen w-full flex items-center justify-center">
                    <div className="w-[35%]  bg-[#035c67aa]  p-4 rounded-lg border-2 border-[#035c67] shadow">
                        ...
                        <CurrentMedicationsForm initialSelectedMedications = {data.medications} />
                    </div>
                </div>
            )}
